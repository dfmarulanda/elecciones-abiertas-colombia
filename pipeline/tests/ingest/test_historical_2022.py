...
import elecciones_pipeline.ingest.historical_2022 as historical
import pytest
import respx
from elecciones_pipeline.cli import app
from elecciones_pipeline.ingest.historical_2022 import (
...
def _zip(payload: str, name: str = "MMV_NACIONAL_PRESIDENTE_2022_1v.csv") -> bytes:
    buffer = BytesIO()
...
def _snapshot(round_number: int = 1) -> Snapshot:
    return Snapshot(
        url=HISTORICAL_2022_URLS[round_number],
        content_hash="a" * 64,
...
def test_parser_rejects_schema_and_invalid_vote_values(payload: str, message: str) -> None:
    with pytest.raises(Historical2022Error, match=message):
...
def test_parser_rejects_extra_missing_duplicate_and_name_conflict() -> None:
    for header in (HEADER.replace(";VOTOS", ";EXTRA;VOTOS"), HEADER.replace(";VOTOS", "")):
        with pytest.raises(Historical2022Error, match="header"):
            list(_rows(_zip(header + ROW), 1, _snapshot()))
    with pytest.raises(Historical2022Error, match="duplicate semantic"):
        list(_rows(_zip(HEADER + ROW + ROW), 1, _snapshot()))
    with pytest.raises(Historical2022Error, match="conflicting names"):
        list(_rows(_zip(HEADER + ROW + ROW.replace("ANTIOQUÍA", "OTRO")), 1, _snapshot()))


def test_parser_rejects_unsafe_zip_shapes() -> None:
    with pytest.raises(Historical2022Error, match="member name"):
        list(_rows(_zip(HEADER + ROW, "other.csv"), 1, _snapshot()))
    with pytest.raises(Historical2022Error, match="member name"):
        list(_rows(_zip(HEADER + ROW, "../../MMV_NACIONAL_PRESIDENTE_2022_1v.csv"), 1, _snapshot()))
    buffer = BytesIO()
    with ZipFile(buffer, "w", ZIP_DEFLATED) as archive:
        archive.writestr("MMV_NACIONAL_PRESIDENTE_2022_1v.csv", (HEADER + ROW).encode("latin-1"))
        archive.writestr("extra.csv", b"x")
    with pytest.raises(Historical2022Error, match="exactly one"):
        list(_rows(buffer.getvalue(), 1, _snapshot()))
    with pytest.raises(Historical2022Error, match="compression ratio"):
        list(_rows(_zip("x" * 3_000_000), 1, _snapshot()))
    with pytest.raises(Historical2022Error, match="valid ZIP"):
    ...
    with pytest.raises(Historical2022Error, match="unsafe MMV ZIP member"):
...
def _mock_payloads(payloads: dict[int, bytes]) -> None:
    for round_number, payload in payloads.items():
        respx.get(HISTORICAL_2022_URLS[round_number]).mock(
            return_value=Response(200, headers={"Content-Type": "application/zip"}, content=payload)
        )


@pytest.mark.asyncio
async def test_fetch_is_conditional_idempotent_and_records_changed_content(tmp_path: Path) -> None:
    first = {1: _zip(HEADER + ROW), 2: _zip(HEADER + ROW, "MMV_NACIONAL_PRESIDENTE_2022_2v.csv")}
    with respx.mock:
        _mock_payloads(first)
        snapshots = await fetch_historical_2022(tmp_path, max_attempts=1)
    with respx.mock:
        for url in HISTORICAL_2022_URLS.values():
            respx.get(url).mock(return_value=Response(304))
        assert await fetch_historical_2022(tmp_path, recheck=True, max_attempts=1) == snapshots
    changed = _zip(HEADER + ROW.replace("0007", "0008"))
    with respx.mock:
        respx.get(HISTORICAL_2022_URLS[1]).mock(
            return_value=Response(
                200, headers={"Content-Type": "application/zip", "ETag": "changed"}, content=changed
            )
        )
        respx.get(HISTORICAL_2022_URLS[2]).mock(return_value=Response(304))
        updated = await fetch_historical_2022(tmp_path, recheck=True, max_attempts=1)
    assert updated[1].content_hash == hashlib.sha256(changed).hexdigest()
    assert updated[1].snapshot_number == 2


@pytest.mark.asyncio
async def test_fetch_retries_and_rejects_redirect_and_type(tmp_path: Path) -> None:
    payloads = {1: _zip(HEADER + ROW), 2: _zip(HEADER + ROW, "MMV_NACIONAL_PRESIDENTE_2022_2v.csv")}
    with respx.mock:
        route = respx.get(HISTORICAL_2022_URLS[1])
        route.side_effect = [
            Response(503),
            Response(200, headers={"Content-Type": "application/zip"}, content=payloads[1]),
        ]
        respx.get(HISTORICAL_2022_URLS[2]).mock(
            return_value=Response(
                200, headers={"Content-Type": "application/zip"}, content=payloads[2]
            )
        )
        assert (await fetch_historical_2022(tmp_path, max_attempts=2))[1].snapshot_number == 1
    with respx.mock:
        respx.get(HISTORICAL_2022_URLS[1]).mock(
            return_value=Response(302, headers={"Location": "https://evil.invalid/a.zip"})
        )
        with pytest.raises(Exception, match="allowlist"):
            await fetch_historical_2022(tmp_path / "redirect", max_attempts=1)
    with respx.mock:
        respx.get(HISTORICAL_2022_URLS[1]).mock(
            return_value=Response(200, headers={"Content-Type": "text/html"}, content=b"PK\x03\x04")
        )
        with pytest.raises(Historical2022Error, match="content type"):
            await fetch_historical_2022(tmp_path / "type", max_attempts=1)
    with respx.mock:
        respx.get(HISTORICAL_2022_URLS[1]).mock(
            return_value=Response(
                200, headers={"Content-Type": "application/zip"}, content=b"not-a-zip"
            )
        )
        with pytest.raises(Historical2022Error, match="not a ZIP"):
            await fetch_historical_2022(tmp_path / "magic", max_attempts=1)
    with respx.mock:
        respx.get(HISTORICAL_2022_URLS[1]).mock(
            return_value=Response(
                200,
                headers={
                    ...
                    "Content-Length": str(historical.MAX_ZIP_BYTES + 1),
                },
                content=b"PK\x03\x04",
            )
        )
        with pytest.raises(Historical2022Error, match="Content-Length"):
            await fetch_historical_2022(tmp_path / "size", max_attempts=1)
...
def test_two_frozen_builds_have_identical_artifact_and_manifest_bytes(
    ...
    raw = {1: _zip(HEADER + ROW), 2: _zip(HEADER + ROW, "MMV_NACIONAL_PRESIDENTE_2022_2v.csv")}
    ...
    monkeypatch.setattr(historical, "fetch_historical_2022", frozen_fetch)
    monkeypatch.setattr(historical, "NATIONAL_CONTROL_TOTALS", {1: 7, 2: 7})
    with pytest.raises(Historical2022Error, match="retired fixed v1"):
        build_historical_2022_release(
            state,
            tmp_path / "historical-2022-mmv-context-v1",
            tmp_path / "retired-manifest",
            git_commit="frozen",
        )
    one = build_historical_2022_release(
        state, tmp_path / "one", tmp_path / "man-one", git_commit="frozen"
    )
    two = build_historical_2022_release(
...
def test_historical_cli_atomically_installs_v2_then_is_no_op(
    ...
    release_id = "historical-2022-mmv-context-v2-0123456789abcdef"
    ...
    def fake_build(
        ...
        rows = output / "historical-2022-mmv.parquet"
        rollups = output / "historical-2022-rollups.parquet"
        geography = output / "historical-2022-geography.parquet"
        metadata = output / "historical-2022-metadata.json"
    ...
    monkeypatch.setattr(cli, "build_historical_2022_release", fake_build)
    release_root = tmp_path / "releases"
    manifest_root = tmp_path / "manifests"
    arguments = [
        "historical-2022-build",
        "--state-dir",
        str(tmp_path / "state"),
        "--release-root",
        str(release_root),
    ...
    target_rows = release_root / release_id / "historical-2022-mmv.parquet"
...
def test_historical_cli_help_marks_v2_non_active_and_loader_paths_are_required() -> None:
    help_result = CliRunner().invoke(app, ["historical-2022-build", "--help"])
    assert help_result.exit_code == 0
    assert "v2 historical candidate" in help_result.output
    assert "never activate" in help_result.output
    load_result = CliRunner().invoke(
        app, ["historical-2022-load-postgres", "--database-url", "postgresql://invalid"]
    )
    assert load_result.exit_code == 2
    assert "--manifest" in load_result.output
    missing_release = CliRunner().invoke(
        app,
        [
            "historical-2022-load-postgres",
            "--database-url",
            "postgresql://invalid",
            "--manifest",
            "candidate.json",
    ...
    assert "--release-dir" in missing_release.output
