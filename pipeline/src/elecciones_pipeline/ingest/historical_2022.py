# ruff: noqa: E501, S608, ASYNC240
"""Safe, reproducible ingestion of the Registraduría 2022 MMV snapshots.
...
HISTORICAL_2022_HOST = "observatorio.registraduria.gov.co"
HISTORICAL_2022_URLS = {
    1: "https://observatorio.registraduria.gov.co/anexos/MMV_NACIONAL_PRESIDENTE_2022_1v.zip",
    2: "https://observatorio.registraduria.gov.co/anexos/MMV_NACIONAL_PRESIDENTE_2022_2v.zip",
}
PARSER_VERSION = "historical-mmv-2022/2.0.0"
TRANSFORM_VERSION = "historical-rollup/2.0.0"
RELEASE_FAMILY = "historical-2022-mmv-context-v2"
...
NATIONAL_CONTROL_TOTALS = {1: 21_442_300, 2: 22_689_034}
MAX_ZIP_BYTES = 8 * 1024 * 1024
MAX_MEMBER_BYTES = 200 * 1024 * 1024
MAX_COMPRESSION_RATIO = 100
_MEMBER_RE = re.compile(r"^MMV_NACIONAL_PRESIDENTE_2022_[12]v\.csv$")
...
class Historical2022Error(ValueError):
...
async def fetch_historical_2022(
        ...
        for round_number, source_url in HISTORICAL_2022_URLS.items():
            ...
            for _redirect_number in range(4):
                ...
                for attempt in range(max_attempts):
                    try:
                        ...
                        if attempt + 1 == max_attempts:
                            raise Historical2022Error("official ZIP request failed after retries") from exc
                ...
                if response is None:  # pragma: no cover - defensive type narrowing
                    raise Historical2022Error("official ZIP request failed without a response")
                ...
                if response.is_redirect:
                    ...
                    if not location:
                        raise Historical2022Error("official ZIP redirect omitted Location")
                ...
                if media_type not in {"application/zip", "application/octet-stream"}:
                    raise Historical2022Error(f"unexpected ZIP content type {media_type!r}")
                declared = response.headers.get("Content-Length")
                if declared is not None and (not declared.isdigit() or int(declared) > MAX_ZIP_BYTES):
                    raise Historical2022Error("official ZIP Content-Length exceeds reviewed safety limit")
                content = response.content
                if len(content) == 0 or len(content) > MAX_ZIP_BYTES:
                    raise Historical2022Error("official ZIP body exceeds reviewed safety limit")
                if not content.startswith(b"PK\x03\x04"):
                    raise Historical2022Error("official response is not a ZIP local-file stream")
                ...
                raise Historical2022Error("official ZIP exceeded reviewed redirect limit")
    return snapshots


def _clean(value: str | None, column: str) -> str:
    if value is None:
        raise Historical2022Error(f"MMV row lacks {column}")
    cleaned = value.strip()
    if not cleaned:
        raise Historical2022Error(f"MMV row has blank {column}")
    return cleaned
...
def _code(value: str | None, column: str) -> str:
    ...
    if (
        ...
        raise Historical2022Error(f"invalid MMV {column} code {cleaned!r}")
...
def _rows(
    ...
    *,
    ...
    try:
        ...
        raise Historical2022Error("raw official object is not a valid ZIP") from exc
    with archive:
        members = archive.infolist()
        if len(members) != 1:
            raise Historical2022Error("MMV ZIP must contain exactly one CSV member")
        member = members[0]
        if member.is_dir() or not _MEMBER_RE.fullmatch(member.filename):
            raise Historical2022Error("unexpected MMV ZIP member name")
        if member.flag_bits & 0x1 or member.file_size <= 0 or member.file_size > MAX_MEMBER_BYTES:
            raise Historical2022Error("unsafe MMV ZIP member")
        if member.compress_size <= 0 or member.file_size / member.compress_size > MAX_COMPRESSION_RATIO:
            raise Historical2022Error("MMV ZIP compression ratio exceeds safety limit")
        with archive.open(member) as binary, TextIOWrapper(binary, encoding="latin-1", newline="") as text:
            ...
            if reader.fieldnames is None or tuple(reader.fieldnames) != _REQUIRED_COLUMNS:
                raise Historical2022Error("MMV CSV header does not match the reviewed explicit schema")
            ...
            for line_number, raw in enumerate(reader, start=2):
                try:
                    ...
                    if not votes_text.isascii() or not votes_text.isdigit():
                        raise Historical2022Error("VOTOS must be a non-negative decimal integer")
                    votes = int(votes_text)
                    if votes > 10_000:
                        raise Historical2022Error("VOTOS exceeds reviewed mesa/category sanity bound")
                    ...
                    if not party.isdigit() or len(party) != 4:
                        raise Historical2022Error("PAR must be a four-digit code")
                    ...
                    if semantic_key in semantic_keys:
                        raise Historical2022Error("duplicate semantic MMV fact")
                    semantic_keys.add(semantic_key)
                    yield {
                        "round": round_number,
                        "election_slug": f"presidencia-2022-round-{round_number}",
                        "election_date": "2022-05-29" if round_number == 1 else "2022-06-19",
                ...
                except (ValueError, Historical2022Error) as exc:
                    raise Historical2022Error(f"invalid MMV CSV row {line_number}: {exc}") from exc
...
def _validate_release_tables(
    ...
    *,
    ...
    if raw_rows != expected_rows:
        raise Historical2022Error("raw parquet row count changed during validation")
    for round_number, mesa_count in expected_mesas.items():
        ...
        if observed != mesa_count:
            raise Historical2022Error(f"round {round_number} mesa scope changed during validation")
    duplicate_geography = int(execute(
        "SELECT count(*) - count(DISTINCT id) FROM geography"
    ).fetchone()[0])
    if duplicate_geography:
        raise Historical2022Error("geographic hierarchy has non-unique round-scoped IDs")
    ...
    """).fetchone()[0])
    if orphan_count:
        raise Historical2022Error("geographic hierarchy has missing parents")
    ...
    if invalid_parent_level:
        raise Historical2022Error("geographic hierarchy has invalid parent levels")
    ...
    """).fetchone()[0])
    if mismatch:
        raise Historical2022Error(f"derived geographic rollup reconciliation failed ({mismatch} exceptions)")
    totals = dict(execute("""
        SELECT round, sum(votes)::BIGINT FROM rollups
        WHERE geography_level = 'national' GROUP BY round
    """).fetchall())
    if totals != NATIONAL_CONTROL_TOTALS:
        raise Historical2022Error(
            f"national totals differ from frozen MMV controls: {totals!r} != {NATIONAL_CONTROL_TOTALS!r}"
        )


def build_historical_2022_release(
    state_directory: Path,
    ...
    *,
    ...
    snapshots = asyncio.run(fetch_historical_2022(state_directory))
    objects = LocalObjectStore(state_directory / "objects")
    release_id = _release_id(snapshots, git_commit)
    if output_directory.name == "historical-2022-mmv-context-v1":
        raise Historical2022Error(
    ...
    rows_path = output_directory / "historical-2022-mmv.parquet"
    schema = pa.schema([
    ...
    import duckdb
    rollups_path = output_directory / "historical-2022-rollups.parquet"
    geography_path = output_directory / "historical-2022-geography.parquet"
    ...
    try:
        ...
        if rollup_count is None or geography_count is None:
            raise Historical2022Error("derived artifact count query returned no result")
    ...
    metadata = {
        ...
        "rollup_rows": rollup_rows, "geography_rows": geography_rows, "reconciliation": {"status": "passed", "checked_facts": counts[1] + counts[2], "exceptions": 0},
    }
    metadata_path = output_directory / "historical-2022-metadata.json"
    ...
    if manifest_path.exists() and manifest_path.read_bytes() != (json.dumps(manifest, ensure_ascii=False, sort_keys=True, separators=(",", ":")) + "\n").encode():
        raise Historical2022Error(f"refusing to overwrite non-identical immutable release {release_id}")
