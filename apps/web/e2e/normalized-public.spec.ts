...
const release = "release-2026-r2-public";
...
test.describe("normalized public release explorer", () => {
  test("analysis is plain-language first and gates research outputs", async ({
    ...
    await page.goto(`/es/analitica?${scope}`);
    await page
      .getByRole("heading", { name: /diagnósticos para especialistas/i })
      .scrollIntoViewIfNeeded();
  ...
  test("anomaly detail keeps detection, explanation, and priority separate", async ({
    ...
    ).toHaveAttribute("href", /release=release-2026-r2-public/);
  ...
  test("selector exposes four published contexts and no candidate release", async ({
    page,
  }) => {
    await page.goto(`/es/resultados?${scope}`);
    await expect(
      page.getByRole("heading", { name: /resultados publicados/i }),
    ).toBeVisible();
    const releases = page.getByLabel("Release");
    await expect(releases.locator("option")).toHaveCount(4);
    await expect(
      page.getByText(/candidate release|release candidato/i),
    ).toHaveCount(0);
    await expect(
      page.getByRole("table", { name: /resultados publicados/i }),
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.getByRole("cell", { name: "Escrutinio", exact: true }).first(),
    ).toBeVisible();
  ...
  test("municipality → polling place → mesa is canonical and shareable", async ({
    ...
    await page.goto(
      `/es/resultados/geografia/MUN-001?${scope}&source=scrutiny&level=polling_place`,
    );
    ...
    await expect(page).toHaveURL(
      new RegExp(
        `/es/resultados/geografia/PLACE-01.*release=${release}.*source=scrutiny`,
      ),
    );
    await expect(
    ...
    await expect(page).toHaveURL(
      new RegExp(
        `/es/resultados/mesa/MESA-001.*release=${release}.*source=scrutiny`,
      ),
    );
    ...
    await expect(
      page.getByRole("cell", { name: "Escrutinio", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("cell", { name: "Escrutinio oficial", exact: true }),
    ).toBeVisible();
  ...
  test("direct mesa URL preserves filters across locale switch", async ({
    ...
    await expect(page).toHaveURL(
      new RegExp(
        `/en/resultados/mesa/MESA-001.*release=${release}.*source=scrutiny`,
      ),
    );
    await expect(page.getByRole("heading", { name: "Mesa 001" })).toBeVisible();
    await expect(
    ...
    await expect(
      page.getByRole("option", { name: "Scrutiny", exact: true }),
    ).toBeAttached();
    await expect(
      page.getByRole("cell", { name: "Official scrutiny", exact: true }),
    ).toBeVisible();
  });
